...
geen lege vlakken) die de site visueel compleet maken tot de definitieve
Yiska-foto's beschikbaar zijn. Elke afbeelding gebruikt de huisstijl-gradient,
architectuur-/glasmotieven, zacht licht, vignet en lichte korrel.

    python3 tools/make_placeholders.py            # -> assets/images/*-placeholder.jpg
"""
import os
import numpy as np
...
def grad(w, h, top, bot, t_dir="diag"):
    ...
    t = t[..., None]
    base = np.array(top)[None, None, :] * (1 - t) + np.array(bot)[None, None, :] * t
    return base
...
def glow(arr, center, color, sigma, strength=1.0):
    ...
    col = np.array(color)[None, None, :]
    # screen-achtige blend
    return arr + (255 - arr) * (g * (col / 255.0))


def vignette(arr, amount=0.38):
    h, w, _ = arr.shape
    yy, xx = np.mgrid[0:h, 0:w]
    d = np.sqrt(((xx - w / 2) / (w / 2)) ** 2 + ((yy - h / 2) / (h / 2)) ** 2)
    v = np.clip(1 - amount * np.clip(d - 0.5, 0, 1.5), 0, 1)[..., None]
    return arr * v


def grain(arr, amount=7.0):
    n = np.random.default_rng(7).normal(0, amount, arr.shape[:2])[..., None]
    return arr + n
...
def soft_overlay(img, draw_fn, blur=0):
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0))
    d = ImageDraw.Draw(ov)
    draw_fn(d, img.size)
    if blur:
...
def base_image(w, h, top, bot, glow_center, glow_color, glow_sigma=0.5, glow_str=0.9):
    arr = grad(w, h, top, bot)
    arr = glow(arr, glow_center, glow_color, glow_sigma, glow_str)
    return arr


def finish(arr, path, quality=84):
    arr = vignette(arr)
    arr = grain(arr)
...
# ---- Motieven ----
def window_grid(d, size, cols, rows, color, lit=0.25, alpha=42, lit_alpha=120, x0=0.05, x1=0.95, y0=0.1, y1=0.95):
    w, h = size
    ...
    for i in range(cols):
        for j in range(rows):
            ...
            d.rectangle([x + pad, y + pad, x + cw - pad, y + ch - pad],
                        fill=color + (a,))
...
def streak(img, size, p0, p1, width, color=(255, 255, 255), alpha=46, blur=22):
    def fn(d, sz):
        w, h = sz
        d.line([(p0[0] * w, p0[1] * h), (p1[0] * w, p1[1] * h)],
               fill=color + (alpha,), width=width)
    return soft_overlay(img, fn, blur=blur)
...
def make_hero():
    ...
    img = overlay(img, lambda d, s: droplet(d, s, 0.30, 0.42, 150, CYAN, 60))
    img = streak(img, (w, h), (0.45, 0.1), (0.7, 0.95), 120, alpha=40, blur=40)
    finish(np.asarray(img).astype(np.float64), f"{OUT}/hero-placeholder.jpg", 86)
...
def make_office():
    ...
    # bureaus / silhouetten onderaan
    def desks(d, s):
        ww, hh = s
        for i, x in enumerate(np.linspace(0.1, 0.85, 4)):
            d.rectangle([x * ww, hh * 0.74, x * ww + ww * 0.12, hh * 0.8], fill=(8, 26, 46, 150))
    img = overlay(img, desks)
    img = streak(img, (w, h), (0.2, 0.0), (0.5, 1.0), 90, alpha=36, blur=34)
    finish(np.asarray(img).astype(np.float64), f"{OUT}/office-cleaning-placeholder.jpg")
...
def make_glass():
    ...
    # squeegee-veeg (schone diagonale baan)
    img = streak(img, (w, h), (0.1, 0.15), (0.85, 0.7), 150, alpha=70, blur=30)
    img = streak(img, (w, h), (0.12, 0.2), (0.87, 0.75), 8, alpha=120, blur=4)
    finish(np.asarray(img).astype(np.float64), f"{OUT}/glass-cleaning-placeholder.jpg")
...
def make_commercial():
    ...
    finish(np.asarray(img).astype(np.float64), f"{OUT}/commercial-cleaning-placeholder.jpg")
...
def make_about():
    ...
    img = Image.fromarray(np.clip(arr, 0, 255).astype(np.uint8), "RGB")
    # zachte, overlappende cirkels (warmte/samenwerking, abstract)
    def circles(d, s):
        ww, hh = s
        for cx, cy, r, a in [(0.3, 0.55, 0.22, 40), (0.5, 0.45, 0.26, 34), (0.68, 0.6, 0.2, 44)]:
    ...
    img = streak(img, (w, h), (0.3, 0.0), (0.6, 1.0), 80, alpha=30, blur=36)
    finish(np.asarray(img).astype(np.float64), f"{OUT}/about-placeholder.jpg")
...
def make_contact():
    ...
    def pin(d, s):
        ...
        d.polygon([(cx - r * 0.7, cy + r * 0.5), (cx + r * 0.7, cy + r * 0.5), (cx, cy + r * 1.9)],
                  fill=(255, 255, 255, 230))
        d.ellipse([cx - r * 0.36, cy - r * 0.36, cx + r * 0.36, cy + r * 0.36], fill=BLUE + (255,))
    img = overlay(img, pin)
    finish(np.asarray(img).astype(np.float64), f"{OUT}/contact-placeholder.jpg")


if __name__ == "__main__":
    os.makedirs(OUT, exist_ok=True)
    print("Premium placeholders genereren…")
